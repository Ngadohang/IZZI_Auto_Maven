...
public class AbstractPageOne {
	protected final Log log;
	WebDriver driver;

	public AbstractPageOne() {
	...
	public void submitToElement(WebDriver driver, String locator) {
		sleepInMilisecond(500);
		getElementByXpath(driver, locator).submit();;
	}
	...
	public void senKeyBoarchToElement(WebDriver driver, String locator, Keys key, String... values) {
		Actions action = new Actions(driver);
		action.sendKeys(getElementByXpath(driver, locator, values), key).perform();
	}
	...
	public void submitElementByJs(WebDriver driver, String locator) {
		JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
	...
	public boolean isElementPresence(WebDriver driver, String locator, String... values) {
	...
	public boolean isElementUnDisplayExistDom(WebDriver driver, String locator, String... values) {
		boolean status = false;
		overrideGlobalTimeout(driver, GlobalConstants.SHORT_TIMEOUT);
		List<WebElement> elements = findElementsByXpath(driver, locator, values);
		overrideGlobalTimeout(driver, GlobalConstants.LONG_TIMEOUT);
	...
	public boolean isImageExist(WebDriver driver, String locator, String attributeName) {
		boolean status = false;
		List<WebElement> elements = findElementsByXpath(driver, locator);
		for (WebElement Element : elements) {
			String linkActual = Element.getAttribute("style");
			String linkExcept = "https://static.foodizzi.com/images/2020/1/23/2001231923_Education2048px43.jpg";
			if (linkActual.contains(linkExcept)) {
				status = true;
			}
		}
		return status;
	...
	public boolean isElementExistInListDisplayed(WebDriver driver, String locator, String... values) {
		boolean status = false;
		List<WebElement> elements = findElementsByXpath(driver, locator, values);
		for (WebElement element : elements) {
	...
	public String getStoreInfoNewOnTable(WebDriver driver, String nameStore, String inforStore) {
		...
		WebElement Element = getElementByXpath(driver, "//td[text()='%s']/following-sibling::td[%s]", nameStore,
				String.valueOf(index));
		return Element.getText();
	}

	public Long getMilisecondLoadPage(WebDriver driver, String url) {
